...
export class IndexProductoComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.socket.on('emit-carrito', (data: any) => {
      this._clienteService.listar_productos_publico('').subscribe(
        response=>{        
          this.productos = response.data;
          this.load_data=false;                     
    ...
    this.socket.on('new-producto-admin', (data: any) => {
      this._clienteService.listar_productos_publico('').subscribe(
        response=>{        
          this.productos = response.data;
          this.load_data=false;                     
    ...
    this.socket.on('new-delete-admin', (data: any) => {
      this._clienteService.listar_productos_publico('').subscribe(
        response=>{        
          this.productos = response.data;
          this.load_data=false;                     
    ...
    this.socket.on('new-descuento-admin', (data: any) => {
  ...
  ngAfterViewInit(): void {
    // Inicializa el control deslizante después de que la vista esté disponible
    var slider: any = document.querySelector('.cs-range-slider-ui');

    noUiSlider.create(slider, {
      start: [250, 600],  // Ajusta según tus necesidades
      connect: true,
      range: {
        'min': 0,
        'max': 1000
